...
public class EchoServerThread implements Runnable {
    ...
    private void funDepozyt(BufferedReader brinp, DataOutputStream out) throws IOException {
        ...
        try {
            ...
            if(amount <= 0)
            {
                out.write("Nie mozesz przeslac takiej kwoty!\r\n".getBytes("UTF-8"));
                return;
            }
            double currentBalance = Double.parseDouble(logins.get(userIndex).get(3));
            currentBalance += amount;
            logins.get(userIndex).set(3, String.format(Locale.US, "%.2f", currentBalance));
            zapiszDoBazy();
            out.write(("Nowe saldo: " + String.format(Locale.US, "%.2f", currentBalance) + "\r\n").getBytes("UTF-8"));
    ...
    private void funTransfer(BufferedReader brinp, DataOutputStream out) throws IOException {
        ...
        if (recipientIndex != -1 && amount <= Double.parseDouble(logins.get(userIndex).get(3))) {
            ...
            logins.get(userIndex).set(3, String.format(Locale.US, "%.2f", senderBalance));
            logins.get(recipientIndex).set(3, String.format(Locale.US, "%.2f", recipientBalance));
            zapiszDoBazy();
            out.write(("Transfer pomyslny, nowe saldo: " + String.format(Locale.US, "%.2f", senderBalance) + "\r\n").getBytes("UTF-8"));
        } else {
    ...
    private void funBalans(DataOutputStream out) throws IOException {
        ...
        double currentBalance = Double.parseDouble(logins.get(userIndex).get(3));
        out.write(("Twoje obecne saldo to: " + String.format(Locale.US, "%.2f", currentBalance) + "\r\n").getBytes("UTF-8"));
    }
